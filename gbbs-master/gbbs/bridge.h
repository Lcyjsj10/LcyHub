...
namespace parlay
{
                                        ...
                                        random r = random())
  {
    ...
    size_t n = s.size();
    T pivot = s[r[0] % n];
    sequence<T> smaller = filter(s, [&](T a)
                                 { return less(a, pivot); });
    if (k < smaller.size())
      return kth_smallest(smaller, k, less, r.next());
    else
    {
      sequence<T> larger = filter(s, [&](T a)
                                  { return less(pivot, a); });
      if (k >= n - larger.size())
  ...
  sequence<char> sequence_to_string(TSeq const &T)
  {
    ...
    auto C = sequence<char>::from_function(m, [&](size_t i)
                                           { return (char)0; });
    ...
    return parlay::filter(make_slice(C), [&](char A)
                          { return A > 0; });
  }
